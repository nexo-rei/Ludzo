...
function DiceFace({ value, size = 40, rolling = false }: { value: number; size?: number; rolling?: boolean }) {
  ...
  const dots = dotMap[Math.max(1, Math.min(6, value))] ?? dotMap[1];
  return (
    <motion.svg
...
function LudoBoard({
  ...
  const buildWaypoints = (
    ...
    if (from === to) {
      pts.push(pieceXY(isP1, to, idx));
    } else if (from === 0) {
      pts.push(pieceXY(isP1, 0, idx), pieceXY(isP1, to, idx));
    } else if (to === 0) {
      pts.push(pieceXY(isP1, from, idx), pieceXY(isP1, 0, idx));
    } else if (to > from) {
      for (let p = from; p <= to; p++) pts.push(pieceXY(isP1, p, idx));
    } else {
      pts.push(pieceXY(isP1, from, idx), pieceXY(isP1, to, idx));
    }
...
export default function LudoGamePage() {
  ...
  const handleRoll = useCallback(async () => {
    ...
    // Animate dice face while waiting for server
    let ticks = 0;
