  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] flex flex-col items-center justify-center relative overflow-hidden">
      <motion.div className="flex flex-col items-center" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5 }}>
        ...
        <p className="text-sm text-[var(--text-muted)] mt-2">{t("splash_tagline")}</p>
        <div className="w-28 h-0.5 overflow-hidden bg-[var(--border)] mt-10 rounded-full" role="progressbar" aria-label={t("loading")}><motion.div className="h-full bg-[var(--accent)]" initial={{ x: "-100%" }} animate={{ x: "100%" }} transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }} /></div>
      </motion.div>
      <p className="absolute bottom-10 text-[10px] tracking-[.2em] text-[var(--text-muted)]">{t("splash_bottom")}</p>
    </div>
  );
}
